...
class TournamentDetailScreen extends StatelessWidget {
  ...
  Widget _buildHeader(BuildContext context, Map<String, dynamic> t) {
    return Container(
      ...
      decoration: BoxDecoration(
        color: context.isDark ? AppColors.purple : Theme.of(context).colorScheme.primary.withValues(alpha: 0.1),
      ),
      child: Stack(
        ...
        children: [
          ...
          Column(
            ...
            children: [
              ...
              Text(
                t['name'] ?? 'Tournament',
                style: TextStyle(color: context.isDark ? Colors.white : context.textPrimary, fontSize: 24, fontWeight: FontWeight.w900, fontFamily: 'Rajdhani'),
              ),
              const SizedBox(height: 4),
              Text(
                '${(t['timeControl'] ?? '').replaceAll('_', ' ')} • ${(t['format'] ?? '').toUpperCase()}',
                style: TextStyle(color: context.isDark ? Colors.white.withValues(alpha: 0.8) : context.textSecondary, fontSize: 14, fontWeight: FontWeight.w500),
              ),
            ],
          ),
        ],
  ...
  Widget _buildStatCards(BuildContext context, Map<String, dynamic> t) {
    return Row(
      children: [
        ...
        Expanded(
          child: _StatCard(
            value: '${(t['registeredPlayers'] as List?)?.length ?? 0}/${t['maxPlayers'] ?? 8}',
  ...
  Widget _buildRegistrationProgress(BuildContext context, Map<String, dynamic> t) {
      ...
      child: Column(
        ...
        children: [
          Row(
            mainAxisAlignment: MainAxisAlignment.spaceBetween,
            children: [
              Text('Registration Progress', style: TextStyle(color: context.textPrimary, fontSize: 16, fontWeight: FontWeight.bold)),
              Text('$regCount / $maxCount', style: TextStyle(color: Theme.of(context).colorScheme.primary, fontSize: 16, fontWeight: FontWeight.w900)),
            ],
          ),
  ...
  Widget _buildRules(BuildContext context, Map<String, dynamic> t) {
    return GlassCard(
      ...
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          Text('Tournament Rules', style: TextStyle(color: context.textPrimary, fontSize: 16, fontWeight: FontWeight.bold)),
          const SizedBox(height: 16),
          _buildRuleItem(context, 'Format: ${(t['format'] ?? '').toUpperCase()}'),
          _buildRuleItem(context, 'Time control: ${(t['timeControl'] ?? '').replaceAll('_', ' ')}'),
          _buildRuleItem(context, 'Late entry not allowed'),
          _buildRuleItem(context, 'Fair-play monitoring enabled', showDivider: false),
        ],
      ),
    );
  }

  Widget _buildRuleItem(BuildContext context, String text, {bool showDivider = true}) {
    return Column(
      children: [
        Row(
          children: [
            ...
            const SizedBox(width: 12),
            Text(text, style: TextStyle(color: context.textSecondary, fontSize: 14)),
          ],
        ),
        if (showDivider)
          Padding(
  ...
  Widget _buildRegisteredPlayers(BuildContext context, Map<String, dynamic> t) {
    ...
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        Text('Registered Players', style: TextStyle(color: context.textPrimary, fontSize: 16, fontWeight: FontWeight.bold)),
        const SizedBox(height: 16),
        if (players.isEmpty)
          Text('No players registered yet.', style: TextStyle(color: context.textSecondary))
        else
          Wrap(
            spacing: 12,
            runSpacing: 12,
            children: players.take(12).map((p) => _PlayerChip(name: p.toString().substring(0, 5), color: AppColors.purpleLight)).toList(),
          ),
      ],
    );
  }
      ...
      child: Row(
        ...
        children: [
          Container(
            ...
            child: Center(
              child: Text(name[0].toUpperCase(), style: const TextStyle(color: Colors.white, fontSize: 10, fontWeight: FontWeight.bold)),
            ),
          ),
          const SizedBox(width: 8),
          Text(name, style: TextStyle(color: context.textPrimary, fontSize: 12, fontWeight: FontWeight.bold)),
        ],
      ),
    );
  }
